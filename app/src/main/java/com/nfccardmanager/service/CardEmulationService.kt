...
class CardEmulationService : HostApduService() {
    ...
    override fun processCommandApdu(commandApdu: ByteArray?, extras: Bundle?): ByteArray {
        if (commandApdu == null) return createErrorResponse()
    ...
    private fun handleSelectAid(commandApdu: ByteArray): ByteArray {
        ...
        return if (commandApdu.contentEquals(expectedSelectCommand)) createSuccessResponse()
               else createErrorResponse()
    }

    private fun handleReadCommand(commandApdu: ByteArray): ByteArray = getEmulatedResponse()
    ...
    private fun getEmulatedResponse(): ByteArray {
        ...
        return if (cardData != null) createDataResponse(cardData) else createUidResponse(uid.hexToBytes())
    }

    private fun createSuccessResponse() = byteArrayOf(0x90.toByte(), 0x00.toByte())
    private fun createErrorResponse()   = byteArrayOf(0x6F.toByte(), 0x00.toByte())
    ...
    private fun createDataResponse(data: ByteArray): ByteArray {
        val responseData = data.copyOf(minOf(data.size, 255))
    ...
    override fun onDeactivated(reason: Int) {
        if (reason == DEACTIVATION_LINK_LOSS) hcePreferences?.clearEmulation()
    }
    ...
    companion object {
        /** Convenience helper — HCE is started implicitly by the system, but calling this
         *  ensures the service is kept alive as a started service in addition to being bound. */
